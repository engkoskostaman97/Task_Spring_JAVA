...
public class AuthService {
    ...
    public RegisterResponse register(RegisterRequest request) {
        ...
        // Cek password dan konfirmasi password harus sama
        if (!request.getPassword().equals(request.getKonfirmasipassword())) {
            throw new RuntimeException("Password dan konfirmasi password harus sama");
        }
    
        User user = new User();
        user.setNamadepan(request.getNamadepan());
        user.setNamabelakang(request.getNamabelakang());
        user.setTelephone(request.getNomortelephone());
        user.setEmail(request.getEmail());
        user.setPassword(passwordEncoder.encode(request.getPassword()));
        user.setKonfirmasipassword(passwordEncoder.encode(request.getKonfirmasipassword()));
    
        userRepository.save(user);
    
        return new RegisterResponse(user.getId(),user.getNamadepan(),user.getNamabelakang(),user.getTelephone(),user.getEmail(),user.getPassword(),user.getKonfirmasipassword());
    }
